...
function ChatUI() {
  ...
  const optionCls = D ? 'text-white' : 'text-slate-800';

  const suggestions = ['How does AI work?','Explain machine learning','Write a cover letter','Debug my React code'];

  return (
    <div className={`h-[100dvh] flex overflow-hidden ${txt}`} style={{ background: bg }}>
      {D && <>
        <div className="gradient-01 absolute z-0" /><div className="gradient-02 absolute z-0" />
        <div className="gradient-03 absolute z-0" /><div className="gradient-04 absolute z-0" />
      </>}
      ...
      {/* ── Sidebar ─────────────────────────────────── */}
      <aside
        className={`flex flex-col shrink-0 z-40 border-r ${sbBorder} transition-all duration-300
                    fixed md:relative h-full ${sidebarOpen ? 'w-64' : 'w-0 overflow-hidden'}`}
        ...
        <div className={`flex items-center justify-between px-4 py-4 border-b ${sbBorder} shrink-0`}>
          ...
          <button onClick={() => setSidebarOpen(false)}
            className={`p-1.5 rounded-lg transition-colors md:hidden ${D ? 'hover:bg-white/10 text-slate-400' : 'hover:bg-slate-100 text-slate-500'}`}>
        ...
        <div className="px-3 pt-3 pb-2 shrink-0">
          <button onClick={startNew}
            className="w-full flex items-center justify-center gap-2 btn-primary rounded-lg px-4 py-2.5 text-sm">
            <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div key={g} className="mb-3">
                ...
                {grouped[g].map(item => (
                  <div key={item.id} onClick={() => loadChat(item)}
                    className={`group flex items-center justify-between px-3 py-2 rounded-lg cursor-pointer transition-colors ${hiItem(currentId === item.id)} ${D ? 'text-slate-300' : 'text-slate-700'}`}>
        ...
        <div className={`px-3 py-3 border-t ${sbBorder} shrink-0`}>
          <div onClick={() => setShowSettings(true)}
            className={`flex items-center gap-2 px-2 py-2 rounded-lg cursor-pointer transition-colors ${D ? 'hover:bg-white/5' : 'hover:bg-slate-100'}`}>
      ...
      <div className="flex-1 flex flex-col min-w-0 relative z-10">
        ...
        {/* Header */}
        <header className={`flex items-center justify-between px-4 py-3 border-b ${hdrBorder} ${hdrGlass} shrink-0`}>
          <button onClick={() => setSidebarOpen(!sidebarOpen)}
            className={`p-2 rounded-lg transition-colors ${D ? 'hover:bg-white/10 text-slate-300' : 'hover:bg-slate-100 text-slate-600'}`}>
        ...
        {/* Chat body */}
        <div className="flex-1 overflow-y-auto w-full max-w-3xl mx-auto px-4 sm:px-6 py-6">
          {chat.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full gap-7">
              <div className="text-center">
                <h1 className={`text-3xl sm:text-5xl font-bold leading-tight ${D ? 'text-white/90' : 'text-slate-800'}`}>
                  How can I help you
        ...
        {/* Input */}
        <div className="w-full max-w-3xl mx-auto px-4 sm:px-6 pb-5 shrink-0">
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-black/70 z-50 flex justify-center items-center backdrop-blur-sm p-4">
          <div className={`rounded-2xl p-6 w-full max-w-md shadow-2xl border ${modalBorder}`} style={{ background: modalBg }}>
            {/* Modal header */}
            <div className="flex items-center justify-between mb-5">
              <h2 className={`text-xl font-bold ${D ? 'text-white' : 'text-slate-900'}`}>Settings</h2>
            ...
            <form onSubmit={handleUpdateSettings} className="flex flex-col gap-4">
              ...
              <div>
                <label className={`block text-xs mb-1.5 font-medium ${labelCls}`}>New Password <span className="opacity-50">(blank = keep current)</span></label>
              ...
              {/* <div>
                <label className={`block text-xs mb-1.5 font-medium ${labelCls}`}>Theme</label>
                <select value={themeMode} onChange={e => setThemeMode(e.target.value)} className={inputCls}>
                  <option value="light" className={optionCls}>☀️ Light</option>
                  <option value="dark"  className={optionCls}>🌙 Dark</option>
                  <option value="system" className={optionCls}>⚙️ System</option>
                </select>
              </div> */}
